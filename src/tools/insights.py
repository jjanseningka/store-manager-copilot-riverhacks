...
from tools.sales import get_sales_vs_forecast, get_declining_articles, get_hfb_performance
from tools.stock import get_stock_alerts, get_availability_risks, get_oos_top_sellers
from tools.margin import get_margin_analysis, get_low_margin_alerts
...
def generate_proactive_insights(store: DataStore, bu_sk: int) -> dict[str, Any]:
    ...
    try:
        ...
        if oos_count > 5:
            oos_names = [f"{x.get('series', '')} {x.get('description', '')}" for x in stock.get("out_of_stock", [])[:3]]
            insights.append({
                "type": "alert",
                "severity": "critical",
    ...
    try:
        ...
        if critical_risks:
            names = [f"{r.get('series', '')} {r.get('description', '')}" for r in critical_risks[:3]]
            insights.append({
                "type": "alert",
                "severity": "critical",
                "category": "availability",
    ...
    try:
        ...
        high_impact_events = [e for e in events if e.get("impact") == "high" and e.get("days_until", 99) <= 7]
